...
/**
 * If the recommendations table is empty, kick off the curation cron
 * once. Returns true when we actually started one (so the page can
 * show a friendlier "loading" message), false otherwise.
 *
 * Dedup: we check the entire table count before triggering. Once the
 * cron starts inserting rows for any band, future visits won't see an
 * empty table for long enough to retrigger. There's a small window
 * (the first 30 seconds or so) where two concurrent visitors could
 * both trigger — accepted, the cost is one redundant curation worth
 * a couple of dollars.
 *
 * Uses Next.js `after()` so the trigger runs after the page response
 * is sent — the visitor isn't waiting for fetch() to round-trip before
 * seeing their page.
 */
async function maybeBootstrapCuration(): Promise<boolean> {
  ...
  // Count rows across the entire table — if any band is populated,
  ...
  // Build the absolute URL we need to fetch. In Vercel deploys, the
  // host header gives us the canonical domain.
  let host: string;
  try {
  ...
  const triggerUrl = `${protocol}://${host}/api/cron/refresh-recommendations?secret=${encodeURIComponent(
    secret,
  )}`;

  // Fire after the response is sent so the visitor doesn't wait. The
  // fetch's response is the curation result (~3-5 minutes); we don't
  // care about it, we just want the request to land.
  after(async () => {
    try {
      await fetch(triggerUrl, { method: 'GET' });
    } catch {
      // Bootstrap failed — next visit will try again. No-op.
    }
  });
